package com.moalosi.student;

import com.moalosi.service.CourseService;
import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;

import java.io.IOException;
import java.io.PrintWriter;

@WebServlet(name = "studentCourseEnrollmentNewCourse", value = "/student-enrollment-new-course")
public class StudentMenuEnrollInACourse extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        response.setContentType("text/html");
        PrintWriter out = response.getWriter();
        out.println("<h4>Course Enrollment</h4>");
        out.println("<div class=\"add-form-container\">");
        out.println("<form action=\"student-menu-enroll-in-a-course-controller\" method=\"post\">");
        out.println("<div class=\"selection-container\">");
        out.println("<select class=\"custom-select\" name=\"courseId\" id=\"selectId\">");

        new CourseService().getCourseList()
                .forEach(course -> out.println("<option value=\"" + course.id() + "\">" + course.name() + " " + course.type() + "</option>"));

        out.println("</select>");
        out.println("<label for=\"selectId\">Course</label>");
        out.println("</div>");
        out.println("<div class=\"button-container\">");
        out.println("<input type=\"submit\" value=\"Enroll\">");
        out.println("</div>");
        out.println("</form>");
        out.println("</div>");
    }
}
